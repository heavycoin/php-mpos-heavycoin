...
if ($user->isAuthenticated()) {
  ...
  if ($bitcoin->can_connect() === true) {
    ...
    $Phase = $bitcoin->getphase();
    $Supply = $bitcoin->getsupply();
    $Mmax = $bitcoin->getmaxmoney();
  ...
  $smarty->assign('NETWORK', array('difficulty' => $dDifficulty,
                                   'block' => $iBlock,
                                   'EstNextDifficulty' => $dEstNextDifficulty,
                                   'EstTimePerBlock' => $dExpectedTimePerBlock,
                                   'BlocksUntilDiffChange' => $iBlocksUntilDiffChange,
                                   'reward' => $Reward,
                                   'phase' => $Phase,
                                   'supply' => $Supply,
                                   'mmax' => $Mmax,
                                   'rewardest' => $RewardEst,
                                   'rewardnextwhen' => $RewardNextWhen));
