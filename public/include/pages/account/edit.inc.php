    ...
    if ( @$_POST['do'] && !$user->checkPin($_SESSION['USERDATA']['id'], @$_POST['authPin'])) {
      ...
      if (isset($_POST['unlock']) && isset($_POST['utype'])) {
        ...
        switch (@$_POST['do']) {
          ...
          case 'updateAccount':
            if (!$config['csrf']['enabled'] || $config['csrf']['enabled'] && $csrftoken->valid) {
              if ($user->updateAccount($_SESSION['USERDATA']['id'], $_POST['paymentAddress'], $_POST['payoutThreshold'], $_POST['donatePercent'], $_POST['email'], $_POST['is_anonymous'], $oldtoken_ea, $_POST['vote'])) {
...
$smarty->assign("maxvotelabel", $bitcoin->getmaxvote());
$smarty->assign("votingphase", $bitcoin->getphase());
