...
class Block extends Base {
  ...
  public function addBlock($block) {
    $stmt = $this->mysqli->prepare("INSERT INTO $this->table (height, blockhash, confirmations, amount, difficulty, time, vote) VALUES (?, ?, ?, ?, ?, ?, ?)");
    if ($this->checkStmt($stmt) && $stmt->bind_param('isiddii', $block['height'], $block['blockhash'], $block['confirmations'], $block['amount'], $block['difficulty'], $block['time'], $block['vote']) && $stmt->execute())
      return true;
    return $this->sqlError();
  }
