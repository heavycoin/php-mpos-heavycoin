...
class User extends Base {
   ...
   * @return bool
   **/
  public function updateAccount($userID, $address, $threshold, $donate, $email, $is_anonymous, $strToken, $vote = 0) {
    $this->debug->append("STA " . __METHOD__, 4);
    $bUser = false;
    ...
    if ($this->config['voting_method'] == 'weighted')
    {
        if ($this->bitcoin->can_connect() !== true)
        {
            $this->setErrorMessage('Unable to connect to RPC server to get maxvote');
            return false;
        }
       if (!(is_numeric($vote) && is_int((int)$vote)) || $vote > $this->bitcoin->getMaxVote() || $vote < 0)
        {
            $this->setErrorMessage('Invalid vote');
            return false;
        }
    }
    ...
    if ($this->config['voting_method'] == 'weighted')
    {
        ...
        $stmt = $this->mysqli->prepare("UPDATE $this->table SET coin_address = ?, ap_threshold = ?, donate_percent = ?, email = ?, is_anonymous = ?, vote = ? WHERE id = ?");
        if ($this->checkStmt($stmt) && $stmt->bind_param('sddsiii', $address, $threshold, $donate, $email, $is_anonymous, $vote, $userID) && $stmt->execute()) {
  ...
  public function getUserData($userID) {
    ...
    $stmt = $this->mysqli->prepare("
      ...
      IFNULL(donate_percent, '0') as donate_percent, coin_address, ap_threshold, vote
      FROM $this->table
      WHERE id = ? LIMIT 0,1");
    if ($this->checkStmt($stmt)) {
