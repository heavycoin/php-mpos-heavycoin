...
{if $VOTINGMETHOD == 'weighted'}
      <fieldset>
        <label>Block Reward Vote</label>
        </br>
        <font size="1" style="margin: 0px -200px;">Maximum allowed  block reward vote for '{$votingphase}' phase is {$maxvotelabel}</font>
        <input type="text" name="vote" value="{$GLOBAL.userdata.vote}" size="20" {if $GLOBAL.twofactor.enabled && $GLOBAL.twofactor.options.details && !$DETAILSUNLOCKED}disabled{/if}/>{/nocache}
      </fieldset>
{/if}
    </div>
    <footer>
